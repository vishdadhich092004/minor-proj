...
const VariantForm: React.FC = () => {
    ...
    if (hooksLoading && !variants.length) return <div className="p-10">{t('products.form_loading')}</div>;
    ...
    return (
        <div className="max-w-xl mx-auto pb-10">
            <button onClick={() => navigate('/variants')} className="mb-4 flex items-center text-gray-600 dark:text-gray-400 hover:text-gray-900">
                <ArrowLeft className="h-4 w-4 mr-1" /> {t('variants.back_to_list')}
            </button>
            
            <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                <h1 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">{isEditMode ? t('variants.edit_title') : t('variants.add_title')}</h1>
                
                <form onSubmit={handleSubmit} className="space-y-6">
                    <Input label={t('variants.form.value_label')} value={name} onChange={e => setName(e.target.value)} required />
                    
                    <Select
                        label={t('variants.form.type_label')}
                    ...
                    <div className="flex justify-end space-x-3 pt-4 border-t">
                        <Button type="button" variant="secondary" onClick={() => navigate('/variants')}>{t('common.cancel')}</Button>
                        <Button type="submit" isLoading={loading}>
                            {isEditMode ? t('variants.form.update_btn') : t('variants.form.create_btn')}
                        </Button>
                    </div>
                </form>
            </div>
